...
if server == servers["MainServer"]:
	hashes = get_hash(".")
	bighash = ""
	for i in hashes:
		bighash += i.hexdigest()

	programhash = hashlib.sha256(bighash.encode()).hexdigest()

#programhash = "CorrectHashLolf74c02cf28d39b883fb4bf815f647dbb86e27bdf7ae583d8a677f7c118dd58bd"
import requests
serverpacket = {}
...
enchash = fernet.encrypt(programhash.encode())
s.sendall(enchash)
...
if loginresults.startswith(b"FAILED"):
	print("Login failed!")
	print(loginresults.decode())
	onexit()
...
with open("testep.py", "w") as f:
	f.write(epbot_data)
hashdata = hashlib.sha256(epbot_data.encode()).hexdigest().encode()
print(hashdata)
hashdata = fernet.encrypt(hashdata)
s.send(hashdata)
data = s.recv(1024)
data = fernet.decrypt(data)
s.close()
if data == b"Redownload!":
	os.remove("bin/epbot.dat")
	messagebox.showerror("ERROR", "An error occurred while loading the program, please restart the program.")
	onexit()
else:
	print(email)
	print(password)
	epbot_data = epbot_data.replace("{email}", email)
	epbot_data = epbot_data.replace("{password}", password)
	print("RUNNING!")
	exec(epbot_data)

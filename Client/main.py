...
datapath = path.replace("chromedriver", "config.dat")
datapath = datapath.replace(".exe", "")

print(f"Chromedriver is installed to path: {path}")
if os.path.isfile(datapath):
	with open(datapath, "rb") as f:
		driverdat = f.read()

	driverdat = base64.b64decode(driverdat)
	driverdat = json.loads(driverdat.decode())

	print(driverdat)
else:
	print("Chromedriver has been freshly installed.")
